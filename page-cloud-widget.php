<?php
/*
Plugin Name: Powie's Page Cloud Widget
Plugin URI: https://powie.de
Description: Widget plugin for displaying a list of posts or pages in the sidebar.
Author: Powie
Version: 0.9.1
...
define("PCW_DOMAIN", "pcw");
load_plugin_textdomain(PCW_DOMAIN, false, basename( dirname( __FILE__ ) ) . '/languages' );
add_action(	'widgets_init',	create_function('', 'return register_widget("PageCloudWidget");'));

/**
 * Page List Widget.
 * @author nigauri
 */
class PageCloudWidget extends WP_Widget {
	/**
	 * constructor.
	 */
	function __construct() {
		$widget_ops = array('description' => __("This widget will display a list of posts or pages.", PCW_DOMAIN));
		parent::__construct(
			false,
			__("Post/Page list", PCW_DOMAIN),
			$widget_ops
		);
	}
	/**
	...
	function form( $instance ) {
        ...
        $link_prefix = $instance['link_prefix'];
        $link_suffix = $instance['link_suffix'];
?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>">
				<?php _e('Title:', PCW_DOMAIN); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
		</p>
		<p>
			<?php _e('Type:', PCW_DOMAIN); ?>
			...
			<label for="<?php echo $this->get_field_id('post_or_page'); ?>_1">
				<?php _e('Pages', PCW_DOMAIN); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>">
				<?php _e('Number:', PCW_DOMAIN); ?>
			</label>
		...
		<p>
            <label for="<?php echo $this->get_field_id('link_prefix'); ?>">
				<?php _e('Link Prefix:', PCW_DOMAIN); ?>
			</label>
			<input size="10" id="<?php echo $this->get_field_id('link_prefix'); ?>" name="<?php echo $this->get_field_name('link_prefix'); ?>" type="text" value="<?php echo esc_attr($link_prefix); ?>" />
            <label for="<?php echo $this->get_field_id('link_suffix'); ?>">
				<?php _e('Link Suffix:', PCW_DOMAIN); ?>
			</label>
			<input size="10" id="<?php echo $this->get_field_id('link_suffix'); ?>" name="<?php echo $this->get_field_name('link_suffix'); ?>" type="text" value="<?php echo esc_attr($link_suffix); ?>" />
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_create_date'); ?>">
				<?php _e('Display created date', PCW_DOMAIN); ?>
			</label>
			<br />
			<label for="<?php echo $this->get_field_id('create_date_prefix'); ?>">
				<?php _e('Prefix:', PCW_DOMAIN); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_update_date'); ?>">
				<?php _e('Display modified date', PCW_DOMAIN); ?>
			</label>
			<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('sort_col'); ?>">
				<?php _e('Order by:', PCW_DOMAIN); ?>
			</label>
			<select id="<?php echo $this->get_field_id('sort_col'); ?>" name="<?php echo $this->get_field_name('sort_col'); ?>">
				<option value="post_title" <?php echo $sort_col == "post_title" ? "selected" : ""; ?>><?php _e('Title', PCW_DOMAIN); ?></option>
				<option value="menu_order" <?php echo $sort_col == "menu_order" ? "selected" : ""; ?>><?php _e('Menu order', PCW_DOMAIN); ?></option>
				<option value="post_date" <?php echo $sort_col == "post_date" ? "selected" : ""; ?>><?php _e('Post date', PCW_DOMAIN); ?></option>
				<option value="post_modified" <?php echo $sort_col == "post_modified" ? "selected" : ""; ?>><?php _e('Modified date', PCW_DOMAIN); ?></option>
				<option value="ID" <?php echo $sort_col == "ID" ? "selected" : ""; ?>><?php _e('ID', PCW_DOMAIN); ?></option>
				<option value="post_author" <?php echo $sort_col == "post_author" ? "selected" : ""; ?>><?php _e('Author', PCW_DOMAIN); ?></option>
				<option value="post_name" <?php echo $sort_col == "post_name" ? "selected" : ""; ?>><?php _e('Post name', PCW_DOMAIN); ?></option>
			</select>
			<select id="<?php echo $this->get_field_id('asc_desc'); ?>" name="<?php echo $this->get_field_name('asc_desc'); ?>">
				<option value="asc" <?php echo $asc_desc == "asc" ? "selected" : ""; ?>><?php _e('Asc', PCW_DOMAIN); ?></option>
				<option value="desc" <?php echo $asc_desc == "desc" ? "selected" : ""; ?>><?php _e('Desc', PCW_DOMAIN); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('include'); ?>">
				<?php _e('Include:', PCW_DOMAIN); ?>
			</label>
			...
			<small><?php _e('Page/Post IDs, separated by commas.', PCW_DOMAIN); ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('exclude'); ?>">
				<?php _e('Exclude:', PCW_DOMAIN); ?>
			</label>
...
<?php
	...
	public function update($new_instance, $old_instance) {
        ...
        $instance['link_prefix'] = $new_instance['link_prefix'];
        $instance['link_suffix'] = $new_instance['link_suffix'];
		return $instance;
	}
	/**
	 * Display this widget.
	...
	function widget( $args, $instance ) {
		extract($args);
		//$title = apply_filters('widget_title', empty($instance['title']) ? ($instance['post_or_page'] == "0" ? __('Post list', PCW_DOMAIN) : __('Page list', PCW_DOMAIN)) : $instance['title'], $instance, $this->id_base);
		echo $before_widget;
		if ( $title )
...
foreach ($posts_or_pages_array as $single_post_or_page) {
    $li = "<li>";
    $li .= $instance['link_prefix'];
	$li .= "<a href=\"" . get_page_link($single_post_or_page->ID) . "\" title=\"" . $single_post_or_page->post_title . "\">";
	$li .= $single_post_or_page->post_title;
	$li .= "</a>";
    $li .= $instance['link_suffix'];
